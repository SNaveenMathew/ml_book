...
- $\implies \alpha^2 \bigg|\bigg|X \bigg[ \frac{\partial L}{\partial w} \bigg]_{w = w(t)}\bigg|\bigg|_2^2 \to 0$. We assume this condition is met $\forall w(t) \in R^{p + 1}$, $X \in R^{N_{train} \times (p + 1)}$
