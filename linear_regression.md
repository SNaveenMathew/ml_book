...
$$l(e; w) = log(L(e; w)) = \sum_{i = 1}^{N}\bigg[log\bigg(\frac{1}{\sqrt{2 \pi \sigma^2}}\bigg) \bigg] + \sum_{i = 1}^{N}\bigg[ \frac{- (e^{(i)})^2}{2 \sigma^2}\bigg]$$
...
$$ \implies l(w(t+1)) \le l(w(t)) - \alpha\bigg(1-\frac{\alpha L}{2}\bigg)\bigg|\bigg|g_t\bigg|\bigg|_2^2 \tag{2.1.4.1.1.2}$$
...
$$\begin{eqnarray}
\implies l(w(3)) - (\alpha l(w(1)) + (1-\alpha) l(w_2)) = y^Ty +\alpha^2 w(1)^T X^TXw(1) - 2 \alpha w(1)^TX^Ty - 2(1-\alpha)w(2)^TX^Ty \nonumber \\
...
For a function $f$ of $w \in R^{1}$ that is minimized at $\hat{w}$, the necessary second order condition is

$$\bigg[\frac{\partial^2 f}{\partial w^2}\bigg]_{w=\hat{W}} \ge 0$$

(strict > is sufficient, but not necessary for minimality). Differentiating equation $2.1.2.1.1.b$ with respect to $W^T$ we get:
...
<span style="background-color: #E8DAEF">Definition: A matrix $A$ is positive definite if for any vector z we have the following: $z^TAz > 0$. A matrix $A$ is positive semi-definite if for any vector z we have the following: $z^TAz \ge 0$</span>

<span style="background-color: #E8DAEF">Proof: $X^TX$ is positive semi-definite. Let $A = X^TX$. To prove: $z^TAz \ge 0 \forall z$</span>

<span style="background-color: #E8DAEF">$$z^TAz = z^TX^TXz = (Xz)^TXz = ||Xz||_2^2 \ge 0$$</span>

<span style="background-color: #E8DAEF">Hence proved</span>
...
Let us assume that the design matrix is orthogonal. Therefore, the Gram/covariance matrix will be a diagonal matrix.
...
- $(I-H)X = IX - HX = X - X = 0_{N_{train} \times (p+1)} \tag{2.1.7.3}$
...
From the model definition we have $E[\epsilon] = 0$(each component has zero mean) and $cov(\epsilon) = \sigma^2I$ (iid components each with variance $\sigma^2$), therefore $E[\epsilon^2] = E[\epsilon\epsilon^T] = Var[\epsilon] + (E[\epsilon])^2 = \sigma^2I$. Substituting in $2.1.8.2.1$, we get:
...
$$\implies var(\hat{y^{*}} - y^{*}) = var(\hat{y^{*}}) + var(y^{*}) = \sigma^2(I + X^{*}(X^TX)^{-1}{X^{*}}^T)$$
...
3. The sample Gram/covariance matrix is invertible. In other words, the independent variables don't exhibit high levels of multicollinearity
